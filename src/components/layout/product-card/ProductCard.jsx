...
export default function ProductCard(props) {
    ...
    const settings = {
      ...
      const handleLikeBtn = () => {
          if(!isLiked){
            return <img  onClick = {() => setIsLiked(!isLiked)} src ={heartBroken} className ="likeIcon" />
          }else{
              return <img  onClick = {() => setIsLiked(!isLiked)} src ={heart} className ="likeIcon" />
          }
      }
    ...
    return (
        <>
            ...
            <Col xs ={8} md ={3} className = "productCard" >{/*doc-id = {props.productId[index]}*/}
            {handleLikeBtn()}
            <Slider {...settings} >
                <div>
